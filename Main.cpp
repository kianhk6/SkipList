...
int main(int argc, char** argv) {
	...
	std::vector<SkipList::Quad*>* trail  = map.findWithTrail("a");
	cout << trail->size();
	std::vector<SkipList::Quad*>::iterator i = trail->begin();
	for(i = trail->begin(); i != trail->end(); i++){
		cout << i.operator*()->entry->getKey() << endl;
	}
